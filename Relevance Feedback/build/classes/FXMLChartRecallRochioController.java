 ...
 * @author richa
 */
public class FXMLChartRecallRochioController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        Platform.runLater(() -> {
            ...
            for (int i = 0; i < dataValueListBefore.size(); i++) {
                beforeRecall.getData().add(new XYChart.Data(String.valueOf(dataValueListBefore.get(i).getId()), Double.parseDouble(dataValueListBefore.get(i).getPercentageRecall())));
            }
           
            for (int i = 0; i < dataValueListBefore.size(); i++) {
                afterRecall.getData().add(new XYChart.Data(String.valueOf(dataValueListBefore.get(i).getId()), Double.parseDouble(dataValueListAfter.get(i).getPercentageRecall())));
            }
