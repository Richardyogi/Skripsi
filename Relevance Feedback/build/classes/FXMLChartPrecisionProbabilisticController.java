 ...
 * @author richa
 */
public class FXMLChartPrecisionProbabilisticController implements Initializable {
    
    @FXML
    private LineChart chart;
    
    
    ObservableList<ProbabilisticValueTester> dataValueListBefore;
    ObservableList<ProbabilisticValueTester> dataValueListAfter;
    ...
    public void initialize(URL url, ResourceBundle rb) {
         Platform.runLater(() -> {
            ...
            for (int i = 0; i < dataValueListBefore.size(); i++) {
                beforePrecision.getData().add(new XYChart.Data(String.valueOf(dataValueListBefore.get(i).getId()), Double.parseDouble(dataValueListBefore.get(i).getPrecision())));
            }
           
            for (int i = 0; i < dataValueListBefore.size(); i++) {
                afterPrecision.getData().add(new XYChart.Data(String.valueOf(dataValueListBefore.get(i).getId()), Double.parseDouble(dataValueListAfter.get(i).getPrecision())));
            }
